...
if not os.path.exists(f"{root_dir}/MLMODELS"):
    os.makedirs(f"{root_dir}/MLMODELS")

# Load the dataset
D_numpy = np.load(f"{root_dir}/DATASETS/D.npy")[:, 0, :]  # We only have one SOAP center
n_samples, n_features = D_numpy.shape
E_numpy = np.array([np.load(f"{root_dir}/DATASETS/E.npy")]).T
F_numpy = np.load(f"{root_dir}/DATASETS/F.npy")
dD_dr_numpy = np.load(f"{root_dir}/DATASETS/dD_dr.npy")[:, 0, :, :, :]  # We only have one SOAP center
r_numpy = np.load(f"{root_dir}/DATASETS/r.npy")
...
if not os.path.isfile(f"{root_dir}/MLMODELS/best_model.pt"):
    ...
    for i_epoch in range(n_max_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), f"{root_dir}/MLMODELS/best_model.pt")
...
model.load_state_dict(torch.load(f"{root_dir}/MLMODELS/best_model.pt"))
model.eval()
...
np.save(f"{root_dir}/DATASETS/r_train_full.npy", r_train_full)
np.save(f"{root_dir}/DATASETS/E_train_full.npy", E_train_full)
np.save(f"{root_dir}/DATASETS/F_train_full.npy", F_train_full)
np.save(f"{root_dir}/DATASETS/E_whole_pred.npy", E_whole_pred)
np.save(f"{root_dir}/DATASETS/F_whole_pred.npy", F_whole_pred)
