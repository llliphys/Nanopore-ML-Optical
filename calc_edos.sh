...
for NANOPORE_NAME in "GRAPHENE"; do # "GRAPHENE" "MOS2_2H"; do
    ...
    for MOLECULE_NAME in "ALA"; do
        ...
        SOURCE_DIR=/scratch/hpc-prf-meop/Projects/2DMaterNanopore/DFT+NEGF+ML/DFT/${NANOPORE_NAME}_NANOPORE_BIOMOL_${MOLECULE_NAME}

        OBJECT_DIR=${ROOT_DIR}/DFT/dos/${NANOPORE_NAME}_NANOPORE_BIOMOL_${MOLECULE_NAME}
        ...
        for THETA_ANGLE in $(seq ${ANGLE_MIN} ${ANGLE_STP} ${ANGLE_MAX}); do
            ...
            for PHI_ANGLE in $(seq ${ANGLE_MIN} ${ANGLE_STP} ${ANGLE_MAX}); do
                ...
                for PSI_ANGLE in $(seq ${ANGLE_MIN} ${ANGLE_STP} ${ANGLE_MAX}); do
                    ...
                    WORK_DIR="SGLPT_THETA_${THETA_ANGLE}_PHI_${PHI_ANGLE}_PSI_${PSI_ANGLE}"

                    mkdir -p ${OBJECT_DIR}/${WORK_DIR}

                    if [[ ! -f "${OBJECT_DIR}/${WORK_DIR}/input.fdf" ]]; then
                        TransferPDB2LOC ${SOURCE_DIR}/${WORK_DIR}/input.fdf ${OBJECT_DIR}/${WORK_DIR}
                    fi 

                    if [[ ! -f "${OBJECT_DIR}/${WORK_DIR}/output" ]]; then
                        TransferPDB2LOC ${SOURCE_DIR}/${WORK_DIR}/output ${OBJECT_DIR}/${WORK_DIR}
                    fi 

                    # if [[ ! -f "${OBJECT_DIR}/${WORK_DIR}/run.sh" ]]; then
                    #     TransferPDB2LOC ${SOURCE_DIR}/${WORK_DIR}/run.sh ${OBJECT_DIR}/${WORK_DIR}
                    # fi

                    if [[ ! -f "${OBJECT_DIR}/${WORK_DIR}/siesta.EIG" ]]; then
                        TransferPDB2LOC ${SOURCE_DIR}/${WORK_DIR}/siesta.EIG ${OBJECT_DIR}/${WORK_DIR}
                    fi

                    if [[ ! -f "${OBJECT_DIR}/${WORK_DIR}/siesta.KP" ]]; then
                        TransferPDB2LOC ${SOURCE_DIR}/${WORK_DIR}/siesta.KP ${OBJECT_DIR}/${WORK_DIR}
                    fi

                    cd ${OBJECT_DIR}/${WORK_DIR}
                    
                    Eig2DOS -f -e -5 -E 5 -s 0.05 -n 1000 -k siesta.KP siesta.EIG > edos.dat
